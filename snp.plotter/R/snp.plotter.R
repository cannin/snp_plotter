...
#' \dontrun{
#' example.dir <- system.file("extdata", package="snp.plotter")
#' output.dir <- tempdir()
#' file.copy(example.dir, output.dir, recursive=TRUE)
#' setwd(file.path(output.dir, "extdata"))
#' 
#' snp.plotter(config.file="config_eps_lines_35.txt")
#' snp.plotter(config.file="config_pdf_lines_7.txt")
#' snp.plotter(config.file="config_pdf_lines_7_marker_lines.txt")
#' snp.plotter(config.file="config_pdf_no_lines_35.txt")
#' snp.plotter(config.file="config.txt")
#' 
#' file.path(output.dir, "extdata")
#' dir(file.path(output.dir, "extdata"))
#' }
#' 
#' @importFrom grDevices pdf postscript dev.cur dev.off 
...
snp.plotter <- function(EVEN.SPACED = FALSE, 
		...
		LD.TYPE = c("dprime", "rsquare"), 
		LD.COLOR.SCHEME = c("heat", "cm", "custom", "topo", "gray"), 
		USE.COLORS = TRUE, 
		COLOR.LIST = NULL, 
		PALETTE.FILE = NULL, 
		IMAGE.TITLE = NULL, 
	...
	if(config.var$DISP.LEGEND) {
		draw.legend(config.var, gbl.var, samples)
	}
...
#' 
#' @param config.file config.file 
#' @param config.var config.var 
#' 
...
#' Retrieve data
#' 
#' @param split.snp.file split.snp.file
#' @param split.hap.file split.hap.file 
#' @param config.var config.var 
#' @param gbl.var gbl.var
#' 
...
#' @importFrom grid grid.xaxis grid.yaxis grid.text unit gpar grid.layout vpTree vpList textGrob grid.edit segmentsGrob
...
#' 
#' @importFrom grid gTree textGrob pointsGrob
draw.legend <- function(config.var, gbl.var, samples) {
...
#' 
#' @importFrom utils read.table
#' @importFrom grDevices heat.colors cm.colors topo.colors gray.colors
#' @importFrom grid gList viewport popViewport pushViewport gTree grid.draw rectGrob textGrob
...
#' 
#' @importFrom utils head
create.color.list <- function(config.var, gbl.var, samples) {
...
#' 
#' @param config.var config.var 
#' @param split.color.list split.color.list
#' @param gbl.var gbl.var
#' @param samples samples
#' 
#' @importFrom utils head
create.symbol.list <- function(config.var, split.color.list, gbl.var, samples) {
